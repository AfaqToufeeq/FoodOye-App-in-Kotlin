package com.app.foodoye.extensions
...
class SmoothCheckBox : View, Checkable {
    ...
    private fun measureSize(measureSpec: Int): Int {
        ...
        var result = 0
        when (specMode) {
            View.MeasureSpec.UNSPECIFIED, View.MeasureSpec.AT_MOST -> result =
    ...
    override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        ...
        mLeftLineDistance = Math.sqrt(Math.pow((mTickPoints!![1].x - mTickPoints!![0].x).toDouble(),
            2.0) + Math.pow((mTickPoints!![1].y - mTickPoints!![0].y).toDouble(), 2.0)).toFloat()
        mRightLineDistance =
            Math.sqrt(Math.pow((mTickPoints!![2].x - mTickPoints!![1].x).toDouble(),
                2.0) + Math.pow((mTickPoints!![2].y - mTickPoints!![1].y).toDouble(), 2.0))
                .toFloat()
    ...
    companion object {
        ...
        private val COLOR_CHECKED = Color.parseColor("#19D793")
        private val COLOR_FLOOR_UNCHECKED = Color.WHITE
